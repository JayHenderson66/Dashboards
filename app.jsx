...
// Adapt an ESPN competitor (other-team) into the shape Monogram expects, so we
// can render an opponent logo + colored fallback the same way we do for ours.
function competitorAsTeam(c) {
  if (!c?.team) return null;
  const known = TEAMS.find(t => String(t.espnId) === String(c.team.id));
  if (known) return known;
  return {
    id: `_${c.team.id}`,
    mono: c.team.abbreviation || (c.team.shortDisplayName || "?").slice(0, 3),
    name: c.team.shortDisplayName || c.team.displayName || "—",
    primary: c.team.color ? `#${c.team.color}` : "#333333",
    secondary: c.team.alternateColor ? `#${c.team.alternateColor}` : "#ffffff",
  };
}

// ESPN exposes team logos in two shapes depending on the endpoint:
//   scoreboard:   competitor.team.logo   (string URL)
//   schedule:     competitor.team.logos  (array of {href, rel})
// Normalize so opponent logos render consistently across current/past/upcoming.
function competitorLogoUrl(c) {
  const t = c?.team;
  if (!t) return null;
  if (typeof t.logo === "string" && t.logo) return t.logo;
  if (Array.isArray(t.logos) && t.logos.length) {
    const def = t.logos.find(l => (l.rel || []).includes("default"));
    return (def || t.logos[0])?.href || null;
  }
  return null;
}
...
function FeaturedGame({ ev, team }) {
  ...
  return (
    <div className="upcoming">
      ...
      <div className="upcoming-opp" style={{ display: "inline-flex", alignItems: "center", gap: 8, marginTop: 6 }}>
        <span>{me?.homeAway === "home" ? "vs" : "@"}</span>
        {oppTeam && <Monogram team={oppTeam} size="sm" logoUrl={competitorLogoUrl(opp)} />}
        <span>{teamShortName(opp)}</span>
      </div>
    </div>
  );
}
        ...
        <div className="card card-clickable clickable" style={{ marginBottom: 14 }} onClick={() => onOpenGame && onOpenGame(featured, team)}>
          <div className="card-header">
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
              {live && <span className="score-card-status live">LIVE</span>}
              {(isCompleted(featured) || live) && <span className="stats-hint">View stats →</span>}
            </div>
          </div>
    ...
    <div className="gs-hero">
      <div className="gs-hero-row">
        <div className="gs-hero-side">
          {aT && <Monogram team={aT} size="lg" logoUrl={competitorLogoUrl(awayC)} />}
          <div style={{ minWidth: 0 }}>
...
function GameStatsContent({ summary, ev, team }) {
  ...
  // Stat lookup tries: boxscore.teams[*].statistics → competitor.statistics → top-level competitor field
  const statFor = (competitor, bs, key) => {
    let v = findStat(bs?.statistics, key);
    if (v != null) return v;
    v = findStat(competitor?.statistics, key);
    if (v != null) return v;
    // Some sports (MLB esp.) expose hits/errors as direct fields on the competitor
    if (competitor && competitor[key] != null) return String(competitor[key]);
    return null;
  };
  ...
  const statRows = statKeys
    .map(([key, label]) => ({
      key, label,
      aV: statFor(awayC, aBs, key),
      hV: statFor(homeC, hBs, key),
    }))
    .filter(r => r.aV != null || r.hV != null);

  const hasStats = statRows.length > 0;
  const players = summary?.boxscore?.players || [];
  const hasBoxScore = players.some(b => (b.statistics || []).some(g => g.athletes?.length));
  const hasLineScore = summary?.header?.competitions?.[0]?.competitors?.some(c => c.linescores?.length);
  const hasAnyDetail = hasStats || hasBoxScore || hasLineScore;
        ...
        {hasStats && (
          <div className="gs-section">
            ...
            <div className="gs-stats-table">
              ...
              {statRows.map(r => (
                <div key={r.key} className="gs-stats-row">
                  <div className="gs-cell left">{r.aV ?? "—"}</div>
                  <div className="gs-cell label">{r.label}</div>
                  <div className="gs-cell right">{r.hV ?? "—"}</div>
                </div>
              ))}
            </div>
          </div>
...
function LineScore({ summary }) {
  ...
  const competitors = comp?.competitors || [];
  const hasLines = competitors.some(c => c.linescores?.length);
  if (!hasLines) return null;
  const periods = Math.max(...competitors.map(c => c.linescores?.length || 0));
  // Detect totals — MLB exposes hits/errors directly; otherwise just runs/score
  const hasHE = competitors.some(c => c.hits != null || c.errors != null);
  return (
    <div className="gs-section">
      <div className="gs-section-label">Line Score</div>
      <div className="gs-box-scroll">
        <table className="gs-line-table">
          <thead>
            <tr>
              <th></th>
              {Array.from({ length: periods }).map((_, i) => <th key={i}>{i + 1}</th>)}
              <th className="total">{hasHE ? "R" : "T"}</th>
              {hasHE && <th className="total">H</th>}
              {hasHE && <th className="total">E</th>}
            </tr>
          </thead>
          <tbody>
            {competitors.slice().sort((a, b) => (a.homeAway === "away" ? -1 : 1)).map(c => {
              const sideT = competitorAsTeam(c);
              return (
                <tr key={c.team?.id}>
                  <td className="line-team">
                    {sideT && <Monogram team={sideT} size="sm" logoUrl={competitorLogoUrl(c)} />}
                    <span>{c.team?.abbreviation || c.team?.shortDisplayName}</span>
                  </td>
                  {Array.from({ length: periods }).map((_, i) => {
                    const ls = c.linescores?.[i];
                    const v = ls?.displayValue ?? ls?.value;
                    return <td key={i}>{v == null ? "" : v}</td>;
                  })}
                  <td className="total bold">{c.score?.value ?? c.score ?? "—"}</td>
                  {hasHE && <td className="total">{c.hits ?? "—"}</td>}
                  {hasHE && <td className="total">{c.errors ?? "—"}</td>}
                </tr>
              );
            })}
          </tbody>
...
function BoxScore({ summary }) {
  const players = summary?.boxscore?.players || [];
  const teamsWithData = players.filter(b => (b.statistics || []).some(g => g.athletes?.length));
  if (!teamsWithData.length) return null;
  return (
    <div className="gs-section">
      <div className="gs-section-label">Box Score</div>
      <div className="gs-box-teams">
        {teamsWithData.map((block, idx) => (
          <TeamBoxScore key={block.team?.id || idx} block={block} />
        ))}
      </div>
    </div>
  );
...
function TeamBoxScore({ block }) {
  const teamObj = block.team || {};
  const sideT = competitorAsTeam({ team: teamObj });
  const groups = (block.statistics || []).filter(g => g.athletes?.length);
  if (!groups.length) return null;

  return (
    <div className="gs-box-team">
      <div className="gs-box-team-head">
        {sideT && <Monogram team={sideT} size="sm" logoUrl={competitorLogoUrl({ team: teamObj })} />}
        <div className="gs-box-team-name">{teamObj.displayName || teamObj.shortDisplayName}</div>
      </div>
      {groups.map((g, i) => <BoxGroup key={(g.name || g.type || "g") + i} group={g} />)}
    </div>
  );
}

function BoxGroup({ group }) {
  const labels = group.labels || [];
  const descriptions = group.descriptions || [];
  const athletes = group.athletes || [];
  const groupName = group.text || group.type || group.name || "Stats";
  if (!labels.length || !athletes.length) return null;

  return (
    <div className="gs-box-group">
      <div className="gs-box-group-label">{groupName}</div>
      <div className="gs-box-scroll">
        <table className="gs-box-table">
          <thead>
            <tr>
              <th className="player-col">Player</th>
              {labels.map((l, i) => (
                <th key={i} title={descriptions[i] || ""}>{l}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {athletes.map((a, i) => {
              ...
              const pos = athlete.position?.abbreviation || a.position?.abbreviation;
              return (
                <tr key={athlete.id || i}>
                  <td className="player-col">
                    <span className="player-name">{athlete.shortName || athlete.displayName || "—"}</span>
                    {pos && <span className="player-pos">{pos}</span>}
                  </td>
                  {(a.stats || []).map((s, j) => <td key={j}>{s}</td>)}
                </tr>
              );
            })}
          </tbody>
